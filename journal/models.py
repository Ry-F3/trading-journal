...
class Trade(models.Model):
    ...
    def save(self, *args, **kwargs):
        if not self.pk:
            # Only update row_number for new instances (not existing ones)
    ...
    @classmethod
    def reset_row_numbers(cls):
        trades = cls.objects.all().order_by('row_number', 'id')
        for index, trade in enumerate(trades, start=1):
            trade.row_number = index
            trade.save()
            print(f"Updated Trade with id {trade.id} and row_number: {trade.row_number}")
